 ...
 * al contrato de la interfaz Board.
 */
const mapBoardResponse = (board: BoardRaw): Board => {
  return {
    ...
    // Mapeo de Owner (basado en tu interface BoardMember['user'])
    owner: board.owner 
      ? { ...board.owner, id: Number(board.owner.id) } 
      : { id: 0, username: 'Sin Propietario', email: '' },

    // Mapeo de Members: Maneja tanto si viene el objeto anidado o plano
    ...
    // Mapeo de Columnas y sus tarjetas
...
export const boardService = {
  ...
  getBoardMembers: async (boardId: string | number): Promise<BoardMember[]> => {
   ...
   searchUsers: async (query: string, excludeBoardId?: string | number): Promise<AuthUser[]> => {
    ...
    // Cambiamos el tipo de retorno a AuthUser[] según tus types
